# train_dreambooth_ssim_comparable.py

import os
import argparse
import itertools # <-- MODIFICATION: Added for chaining parameters
import pandas as pd
import torch
...
import bitsandbytes as bnb # <-- MODIFICATION: Import 8-bit optimizer library
...
def parse_args():
    ...
    # <-- MODIFICATION: Added gradient_accumulation_steps to match Code 1
...
def main():
    ...
    # <-- MODIFICATION: Switched to 8-bit AdamW and training full UNet + Text Encoder
    optimizer = bnb.optim.AdamW8bit(
        itertools.chain(unet.parameters(), text_encoder.parameters()),
        lr=args.learning_rate,
    )

    # <-- MODIFICATION: Set num_warmup_steps to 0 to match Code 1
    ...
    for epoch in range(args.num_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with torch.cuda.amp.autocast():
                ...
                # <-- MODIFICATION: Account for gradient accumulation
                loss = loss / args.gradient_accumulation_steps

            scaler.scale(loss).backward()

            # <-- MODIFICATION: Implement gradient accumulation logic
