...
def parse_args():
    parser = argparse.ArgumentParser(description="DreamBooth fine-tuning with SSIM and Dice loss.")
    ...
    parser.add_argument("--output_dir", type=str, default="./results_dreambooth_combined")
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        for step, batch in enumerate(dataloader):
            ...
            with torch.amp.autocast('cuda'):
                ...
                # Reconstruct x0 & Decode VAE (Grads Allowed)
                ...
                # Normalize [-1, 1] -> [0, 1] for Metrics
                pred_img = (pred_img / 2 + 0.5).clamp(0, 1)
                
                # SSIM Loss (Optimization: Only compute if weight > 0)
                ssim_loss = torch.tensor(0.0).to(device)
                if args.ssim_loss_weight > 0:
                    target_img = (batch["pixel_values"].chunk(2, dim=0)[0].to(device) / 2 + 0.5).clamp(0, 1)
                    ssim_val = ssim(pred_img, target_img, data_range=1.0)
                    ssim_loss = 1.0 - ssim_val
                    writer.add_scalar("Metric/SSIM", ssim_val.item(), global_step)

                # Dice Loss
                dice_loss = dice_loss_fn(seg_model(transforms.functional.rgb_to_grayscale(pred_img)), batch["masks"].to(device))
                total_loss = db_loss + args.dice_loss_weight * dice_loss + args.ssim_loss_weight * ssim_loss
            ...
            pbar.set_postfix(loss=total_loss.item(), ssim=ssim_loss.item() if args.ssim_loss_weight > 0 else 0); pbar.update(1); global_step += 1
