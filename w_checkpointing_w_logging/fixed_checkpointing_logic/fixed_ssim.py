...
def main():
    ...
    model_source_path = args.model_id # Default source
    ...
    if os.path.isdir(args.output_dir):
        ...
        if checkpoint_dirs:
            ...
            if latest_checkpoint_dir:
                ...
                model_source_path = checkpoint_path # Load models from checkpoint
                ...
                if os.path.exists(state_path):
                    ...
                    scheduler_state = training_state.get("lr_scheduler")
                    scaler_state = training_state.get("scaler")
                    
                    initial_epoch = training_state.get("epoch", latest_epoch)
                    global_step_resume = training_state.get("global_step", latest_epoch * (len(os.listdir(args.instance_data_dir)) // args.batch_size))
                    
                    print(f"Resuming from epoch {initial_epoch} and global step {global_step_resume}")
                else:
                    print(f"Warning: No 'training_state.pt' found in {checkpoint_path}. Resuming models but not optimizer/scheduler state.")
                    initial_epoch = latest_epoch
                    global_step_resume = initial_epoch * (len(os.listdir(args.instance_data_dir)) // args.batch_size)
    # --- END OF RESUME LOGIC ---

    print(f"Loading models from: {model_source_path}")

    # --- [FIXED] MODEL LOADING LOGIC ---
    # Determine subfolder based on source.
    # The base model ("stabilityai/...") has subfolders.
    # Our saved checkpoints have all components in the root directory (subfolder=None).
    tokenizer_subfolder = "tokenizer" if model_source_path == args.model_id else None
    text_encoder_subfolder = "text_encoder" if model_source_path == args.model_id else None
    unet_subfolder = "unet" if model_source_path == args.model_id else None

    tokenizer = CLIPTokenizer.from_pretrained(model_source_path, subfolder=tokenizer_subfolder)
    text_encoder = CLIPTextModel.from_pretrained(model_source_path, subfolder=text_encoder_subfolder).to(device)
    
    # Add new token only if not resuming (or if token is missing)
    ...
    unet = UNet2DConditionModel.from_pretrained(model_source_path, subfolder=unet_subfolder).to(device)
    ...
    if optimizer_state:
        try:
            ...
            print(f"Could not load optimizer state: {e}. Using new optimizer.")
    if scheduler_state:
        try:
            lr_scheduler.load_state_dict(scheduler_state)
            print("Loaded scheduler state.")
        except Exception as e:
            print(f"Could not load scheduler state: {e}. Using new scheduler.")
    if scaler_state:
        try:
            scaler.load_state_dict(scaler_state)
            print("Loaded scaler state.")
        except Exception as e:
            print(f"Could not load scaler state: {e}. Using new scaler.")
    ...
    for epoch in range(initial_epoch, args.num_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with torch.amp.autocast('cuda'):
                ...
                # This is the line where the weights are applied
